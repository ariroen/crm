...
def format_card(c) -> str:
    ...
    name = hd.bold(c.full_name)
    lines = [
        "🪖 <b>Карточка #{}</b>".format(c.id),
        "━━━━━━━━━━━━━━━━━━━━━",
        "👤 {}".format(name),
    ]
    if c.phone:
        lines.append("📞 <code>{}</code>".format(hd.quote(c.phone)))
    if c.source:
        lines.append("📺 Источник: <i>{}</i>".format(hd.quote(c.source)))
    if hasattr(c, 'category') and c.category:
        lines.append("📁 <b>{}</b>".format(hd.quote(c.category.name)))
    ...
    if c.notes:
        ...
        lines.append("\n📝 <i>{}</i>".format(safe_notes))
    lines.append("━━━━━━━━━━━━━━━━━━━━━")
    lines.append("🕐 <code>{}</code>".format(c.created_at.strftime('%d.%m.%Y %H:%M')))
    return "\n".join(lines)
...
async def cb_search_start(callback: CallbackQuery, state: FSMContext):
    await state.set_state(CandidateFSM.waiting_search_query)
    await callback.message.edit_text("🔍 Введите имя или телефон для поиска:", parse_mode="Markdown")
    await callback.answer()


@router.message(CandidateFSM.waiting_search_query)
async def process_search(message: Message, state: FSMContext, session: AsyncSession):
    ...
    await state.clear()
    svc = CandidateService(session)
    candidates = await svc.search(query)

    if not candidates:
        await message.answer(f"🔍 По запросу «{query}» ничего не найдено.", reply_markup=main_menu_kb())
        return

    await message.answer(
        f"🔍 **Результаты поиска ({len(candidates)}):**",
        reply_markup=candidates_list_kb(candidates), parse_mode="Markdown"
    )
