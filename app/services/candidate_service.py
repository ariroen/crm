...
class CandidateService:
    ...
    async def process_ai_data(self, intent: str, data: dict, user_id: int) -> tuple[str, Candidate | None]:
        ...
        if intent == "create_candidate":
            ...
            name = data.get("full_name", "")
            if not name:
                return "⚠️ Укажите, кого искать.", None
            candidates = await self.search(name)
            if not candidates:
                return f"🔍 Кандидат «{name}» не найден.", None
            lines = [c.status_line for c in candidates[:10]]
            return "🔍 **Результаты поиска:**\n" + "\n".join(lines), candidates[0]

        elif intent == "list":
            candidates = await self.list_active()
            if not candidates:
                return "📋 Список кандидатов пуст.", None
            lines = [f"`{c.id}.` {c.status_line}" for c in candidates[:20]]
            return "📋 **Активные кандидаты:**\n" + "\n".join(lines), None
