...
async def check_stale_candidates(bot, admin_chat_id: int, hours=48):
    """Уведомление о зависших — если кандидат не обновлялся >48ч.
    Уведомляет закреплённых операторов и админа.
    """
    async with async_session() as session:
        ...
        by_op = {}  # op_id -> [candidates]
        no_op = []

        for c in stale:
            if c.assigned_operator_id:
                if c.assigned_operator_id not in by_op:
                    by_op[c.assigned_operator_id] = []
                by_op[c.assigned_operator_id].append(c)
            else:
                no_op.append(c)

        from app.db.models import TicketStatus, MedicalStatus, Operator

        async def send_to_user(target_chat_id, candidates, title_prefix=""):
            lines = ["{}ЗАВИСШИЕ КАНДИДАТЫ ({})".format(title_prefix, len(candidates)),
                     "━━━━━━━━━━━━━━━━━━━━━",
            ...
            try:
                ...
                logger.error("❌ Ошибка уведомления %s: %s", target_chat_id, e)

        # 1. Уведомляем операторов
        for op_id, candidates in by_op.items():
            op = await session.get(Operator, op_id)
            if op and op.user_id:
                await send_to_user(op.user_id, candidates, "👤 ТВОИ ")

        # 2. Уведомляем админа о тех, у кого нет оператора, или о всех сразу
        if no_op:
            await send_to_user(admin_chat_id, no_op, "⚠️ БЕЗ ОПЕРАТОРА: ")
        
        logger.info("⚠️ Уведомления о зависших (%d всего) разосланы", len(stale))
